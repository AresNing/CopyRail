...
- **选择界面语言。** 在「设置 → 通用 → 语言」切换简体中文和 English，立即保存，下次启动继续使用。
...
在 **通用 → 语言** 切换界面语言会立即生效，不会顺带保存其他尚未保存的设置。复制内容、分类名称和用户输入不会被翻译。
...
CopyRail 是**早期源码 MVP**，主要在 Apple Silicon macOS 上开发和测试。当前本地迭代为 **0.1.0-local-beta.14**，尚未提供经过 Developer ID 签名和公证的二进制发行版。
