...
*演示界面使用 English、**50% 背景透明度**，配合合成测试数据和演示背景。[截图说明](docs/images/README.md)。*
...
在 **通用 → 背景透明度** 调整主界面、设置页和预览页的毛玻璃背景：0% 为不透明，100% 为通透的 macOS 毛玻璃，默认 50%，本文截图使用 50%。调整自动保存，文字和按钮保持清晰；系统的减少透明度设置优先。设置页统一使用系统字体和标题、正文、说明三级字号。
