...
*演示界面使用 English、**完全不透明背景（透明度 0%）**，配合合成测试数据和演示背景。[截图说明](docs/images/README.md)。*

## 可以做什么

- **查找复制过的内容。** 搜索历史，并按内容类型、来源应用、设备或时间筛选。
- **整理常用片段。** 创建分类，重命名条目、编辑文本，调整保存内容的顺序。
- **选择打开时的位置。** 设置 → 通用 → 打开时定位，可选择“最新内容”（默认）或“上次停留的位置”。后者记住选中的卡片及搜索、筛选和分类上下文，重启后仍可恢复；新增内容不会替换记住的卡片。内容已删除或过期时回到最新历史。选项自动保存，下次唤起生效。
...
在 **通用 → 语言** 切换界面语言会立即生效。设置自动保存：开关点击即生效，数字和应用列表在输入短暂停顿后保存；无效输入会提示并保留原有设置，无需点击保存按钮。“跟随系统”会在启动时读取 macOS 首选语言，可随时切回以取消手动指定。复制内容、分类名称和用户输入不会被翻译。

在 **通用 → 背景透明度** 调整主界面、设置页和预览页的毛玻璃背景：0% 为不透明，100% 为通透的 macOS 毛玻璃，默认 50%，本文截图使用完全不透明背景（0%）。调整自动保存，文字和按钮保持清晰；系统的减少透明度设置优先。设置页统一使用系统字体和标题、正文、说明三级字号。
