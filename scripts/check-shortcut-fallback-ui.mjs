...
await withCompiledUiTest(async ({ page, evaluate, waitFor, fixture, screenshot, browser, artifacts }) => {
  ...
  for (const compact of [true, false]) for (const theme of ['dark', 'light']) {
    ...
    await waitFor(`document.querySelectorAll('.clip-card').length === 5 && document.querySelector('.shortcut-hint')?.textContent === '快捷键未启用'`);
    ...
    await evaluate(`document.querySelector('.shortcut-hint').focus()`);
    ...
    const geometry = await evaluate(`(() => {
      const hint=document.querySelector('.shortcut-hint').getBoundingClientRect();const panel=document.querySelector('.settings-popover').getBoundingClientRect();
    ...
    await waitFor(`document.querySelector('.shortcut-state')?.textContent === '已注册' && document.querySelector('.shortcut-hint')?.textContent === '⇧⌘V'`);
  ...
  await evaluate(`document.querySelector('.shortcut-hint').click()`);
